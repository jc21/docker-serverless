...
RUN apk --no-cache add python python3 python3-dev py-pip ca-certificates \
	groff less bash make jq curl wget g++ zip git openssh && \
...
RUN mkdir -p /tmp/yarn /opt/yarn/dist
WORKDIR /tmp/yarn
